...
class StartingDifficulty(Choice):
    ...
    Guilty: The base Difficulty. What you normally play on if you have never beaten TiTN (Trapped in the Nightmare),
    so probably the most familiar to you out of all of them.
    ...
    other Threats are slightly less agressive and you start with some spare power.
    NOTICE: This one requires you to beat BiD (Burning) it Down) to unlock. If you haven't beaten it yet, you don't
    have it unlocked, and therefore you probably shouldn't choose it as your Starting Difficulty.
    
    If you're wondering "Where's Lost"? It doesn't count. You (most likely) can't even beat the game. If I ever
    added it to the Manual, it'd probably just be a Trap, to be honest. Maybe I should do that at some point,
    now that I think about it. Maybe when the game gets updated in November/December.
    """
    option_calm = 0
    option_guilty = 1
    option_devastated = 2
...
class Fishsanity(Choice):
    ...
    They all have an equal 3% chance to be fished up.
    
    Colors and Characters: The 10 colored fish and 26 character fish are Checks, totaling to 36 Checks.
    Note that this also includes the Ticket and Fish Emoji...because yes.
    
    True Fishsanity: EVERY fish is a Check.
    CAUTION: This includes the 4 rarest fish in the game, those being the Paintbrush Fish, Note in a Bottle, The Ferryman, and the Void Fish, each of which having a <2% chance of appearing. Be aware that completing your Checks may take a while with this enabled.
    (44 Checks)
    """
    option_disabled = 0
